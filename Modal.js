...
const Modal = ({ isOpen, onClose, prompt }) => {
  ...
  return (
    <div
      // Ensure sufficient padding around the modal container on all devices (p-4)
      className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center p-4"
      onClick={onClose}
    >
      <div
        // 1. Reduced max height on mobile (max-h-full)
        // 2. Used max-w-lg (large) on mobile and expanded to max-w-5xl on desktop (md:)
        className="bg-white rounded-xl w-full max-w-lg md:max-w-5xl max-h-full md:h-[85vh] relative flex flex-col overflow-y-auto"
      ...
      >
        ...
        {/* Added internal padding (p-4 on mobile, p-6 on md+) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 p-4 md:p-6 h-full overflow-y-auto md:overflow-hidden">
          ...
          <div className="flex items-center justify-center bg-gray-100 rounded-lg overflow-hidden h-64 md:h-auto md:max-h-full">
            <img
              ...
              // Ensure image scales down nicely
          ...
          <div className="flex flex-col h-full overflow-hidden">
            ...
            {/* Scrollable Description Box - Fixed Height adjusted for mobile space */}
            {/* Increased vertical flexibility by reducing fixed height on mobile */}
